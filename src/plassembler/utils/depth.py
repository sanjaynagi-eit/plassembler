            ...
            for chunk in chunks:
                for repName, group in chunk.groupby("contig", sort=False):
                    if repName not in depths:
                        # the bam was made by mapping against exactly these
                        # contigs, so this means the fasta and the bam disagree.
                        # The old code hit a KeyError here; keep failing loudly,
                        # just with a message that says what went wrong.
                        raise KeyError(
                            f"{bam_file} contains alignments to contig "
                            f"'{repName}', which is not in the reference fasta"
                        )
