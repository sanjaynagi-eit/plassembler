...
def mash_tophits_df(mash_tsv, contig_count):
    ...
    """
    contigs = pd.DataFrame({"contig": range(1, contig_count + 1)})

    if is_file_empty(mash_tsv):
        tophits = contigs.copy()
        for column in TOPHITS_COLUMNS[1:]:
            tophits[column] = ""
        return tophits
    ...
    tophits = contigs.merge(best, on="contig", how="left")
    # contigs with no mash hit keep the blank strings the old loop appended
    return tophits[TOPHITS_COLUMNS].fillna("")
