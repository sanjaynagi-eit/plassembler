...
def test_integer_mash_pval_is_not_promoted_to_float(tmp_path):
    """mash reports pval 0 for a strong hit. Assembling the table with a left
    merge against 1..contig_count introduces NaN for contigs without a hit,
    which promotes the int column to float and writes "0.0" into
    <prefix>_summary.tsv instead of "0"."""
    tsv = write_mash(
        tmp_path / "mash.tsv",
        [[2, "NZ_CP124653.1", 0.0450347, 0, "241/1000"]],
    )
    # contig 1 has no hit, so it is the row that would introduce the NaN
    df = mash_tophits_df(tsv, 2)
    rendered = df.to_csv(index=False, sep="\t")
    assert "\t0\t241/1000" in rendered, rendered
    assert "0.0\t241/1000" not in rendered


def test_contig_column_stays_integral(tmp_path):
    """Same promotion hazard for the contig ids themselves."""
    tsv = write_mash(tmp_path / "mash.tsv", [[3, "ACC", 0.01, 0, "1/1000"]])
    df = mash_tophits_df(tsv, 3)
    assert list(df["contig"]) == [1, 2, 3]
    assert "1.0" not in df.to_csv(index=False)
